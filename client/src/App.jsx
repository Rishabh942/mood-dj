...
useEffect(() => {
  ...
  const getRecs = async () => {
    ...
    const params = new URLSearchParams({
      ...
      target_valence: (mode === "change" ? flip(base.target_valence) : base.target_valence).toString(),
      target_energy: (mode === "change" ? Math.max(0, Math.min(1, 1 - base.target_energy + 0.1)) : base.target_energy).toString(),
  ...
  const createPlaylist = async () => {
    ...
    const r = await axios.post(`${API}/api/create-playlist`, { uris, name: `Mood DJ - ${emotion}` });
  ...
  return (
    <div style={{ maxWidth: 900, margin: "32px auto", fontFamily: "Inter, system-ui" }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        <button onClick={connectSpotify}>{authed ? "Re-connect Spotify" : "Connect Spotify"}</button>

        <label>
          Mood:&nbsp;
          <select value={emotion} onChange={e => setEmotion(e.target.value)}>
            {Object.keys(EMOTION_TO_FEATURES).map(m => <option key={m} value={m}>{m}</option>)}
          </select>
        </label>
      ...
      <div style={{ marginTop: 24 }}>
        <h2>Tracks</h2>
        {tracks.length === 0 && <p>No tracks yet. Connect Spotify, pick a mood, then “Get Recommendations.”</p>}
        {tracks.map(t => (
          <div key={t.id} style={{ padding: 12, border: "1px solid #333", borderRadius: 8, marginBottom: 10 }}>
            <div><b>{t.name}</b> — {t.artists.map(a => a.name).join(", ")}</div>
            <div>Album: {t.album.name}</div>
            <a href={t.external_urls.spotify} target="_blank" rel="noreferrer">Open in Spotify</a>
          </div>
        ))}
      </div>
    </div>
